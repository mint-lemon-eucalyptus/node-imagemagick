...
function exec2(file, args /*, options, callback */) {
    var options = { encoding: 'utf8', timeout: 0, maxBuffer: 500 * 1024, killSignal: 'SIGKILL', output: null
    };
    ...
    var proc = exec2(exports.identify.path, args, {timeout: 120000}, function (err, stdout, stderr) {
        ...
        if (!err) {
            if (isCustom) {
                ...
                if (!result.geometry) {
                    console.log('inside imagemagick', 'called .identify', pathOrArgs, result['geometry']);
                }
